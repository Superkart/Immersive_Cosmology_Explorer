...
public class dataImporter1s : MonoBehaviour
{
    ...
    private readonly List<Material> pointCloudMaterials = new List<Material>();
    ...
    private void ImportPointCloud(int frameNum, GameObject parent)
    {
        ...
        pointCloudMaterials.Add(meshRenderer.material);
    }
    ...
    public void UpdatePointSize(float ui)
    {
        float size = Mathf.Lerp(0.1f, 1.0f, Mathf.Clamp01(ui));
        foreach (var m in pointCloudMaterials)
            if (m && m.HasProperty("_Size")) m.SetFloat("_Size", size);
        //Debug.Log($"[Size] ui={ui:F3} -> _Size={size:F3} on {pointCloudMaterials.Count} mats");
    }

    public void UpdatePointTransparency(float ui)
    {
        float a = Mathf.Clamp01(ui);
        foreach (var m in pointCloudMaterials)
        {
            if (m && m.HasProperty("_AlphaMultiplier")) m.SetFloat("_AlphaMultiplier", a);
            if (m && m.HasProperty("_Alpha")) m.SetFloat("_Alpha", a);
        }
      //  Debug.Log($"[Alpha] ui={ui:F3} -> {a:F3}");
    }
